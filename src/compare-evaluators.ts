...
async function runComparison(): Promise<void> {
  ...
  // Map evaluators to their company's models
  const evaluatorToCompanyModels: Record<string, string[]> = {
  ...
  for (const evaluator of evaluators) {
    const companyModels = evaluatorToCompanyModels[evaluator] || [];
    ...
    for (const result of results) {
      ...
      const isOwnCompany = companyModels.some(m =>
        result.model.toLowerCase().includes(m.toLowerCase().replace(/:/g, ""))
      );

      // Simpler check - match by company name in model string
...
async function generateDetailedReport(
  ...
  for (let i = 0; i < Math.min(5, sortedByVarianceAsc.length); i++) {
    const { model, variance, byEvaluator, consensus } = sortedByVarianceAsc[i];
    const shortModel = model.split("/")[1] || model;
